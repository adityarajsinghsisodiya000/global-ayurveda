      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-12 border-b border-[#143621]">
          ...
          <div className="lg:col-span-1">
            ...
            <button onClick={() => document.getElementById('contact-form')?.scrollIntoView({ behavior: 'smooth' })} className="mt-4 w-full bg-[#092215] hover:bg-[#0e3820] border border-[#d4af37]/50 text-[#d4af37] font-bold text-[10px] py-2 rounded shadow tracking-widest uppercase transition-all flex items-center justify-center gap-2">
        ...
        <div id="contact-form" className="py-10 border-b border-[#143621]">
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl">
              ...
              <div>
                <input type="tel" name="phone" placeholder="Your Phone (10 digits)" maxLength={10} inputMode="numeric" value={formData.phone} onChange={(e) => { if (/^\d{0,10}$/.test(e.target.value)) handleChange(e); }} className={`w-full bg-[#0a2315] border ${errors.phone ? 'border-red-500' : 'border-[#1b482d]'} rounded px-3.5 py-2 text-xs text-white placeholder-gray-400 focus:outline-none focus:border-[#d4af37] transition-colors`} />
                {errors.phone && <p className="text-[10px] text-red-400 mt-1">{errors.phone}</p>}
              </div>
              <div>
                <textarea name="message" rows={2} placeholder="Your Message" value={formData.message} onChange={handleChange} className={`w-full bg-[#0a2315] border ${errors.message ? 'border-red-500' : 'border-[#1b482d]'} rounded px-3.5 py-2 text-xs text-white placeholder-gray-400 focus:outline-none focus:border-[#d4af37] transition-colors resize-none`} />
              </div>
              <div className="sm:col-span-2">
                <button type="submit" className="w-full sm:w-auto bg-[#d4af37] hover:bg-[#e6c24d] text-[#05140c] font-bold text-xs py-2.5 px-8 rounded shadow tracking-widest uppercase transition-all flex items-center justify-center gap-2">
